...
def show():
    ...
    plt.plot([x+1 for x in range(len(switch_list))],
             switch_list, label="Switching", color="purple")
    plt.plot([x+1 for x in range(len(not_switch_list))],
             not_switch_list, label="Not Switching", color="orange")

    avg_switch = sum(switch_list)/len(switch_list)
    avg_not_switch = sum(not_switch_list)/len(not_switch_list)

    print("{:45s}: {:.2f}%".format(
        "Average Switching's Winning percentage", avg_switch))
    print("{:45s}: {:.2f}%".format(
        "Average Not Switching's Winning percentage", avg_not_switch))
